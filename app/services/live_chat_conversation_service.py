...
class LiveChatConversationService:
    ...
    def build_choice_image_prompt(self, context: dict, choice: dict) -> str:
        ...
        prompt = (
            ...
            "キャラクターの感情が一目で分かる表情、視線、手元、肩や姿勢の変化を必ず描く。"
            "背景説明だけで終わらせず、その選択によってキャラクターの内側で何が動いた瞬間なのかを見せる。"
            "嬉しい、照れ、安心、戸惑い、期待、寂しさ、興味、緊張など、会話文脈に合う感情を顔と身体の演技で表現する。"
    ...
    def _build_location_move_prompt(self, context: dict, location) -> str:
        ...
        lines = [
            ...
            "施設紹介画像ではなく、キャラクターがその場所に来て感情を動かした瞬間のイベントCGにする。",
            "主役は施設そのものではなく、キャラクターの反応。背景は感情を引き立てる舞台として使う。",
            "キャラクターの表情、視線、手元、肩や姿勢に、嬉しい、照れ、安心、戸惑い、期待、興味、緊張など会話文脈に合う感情を必ず出す。",
            "登録済みキャラクターの顔、髪型、衣装の印象、画風の連続性を保つ。",
            "セリフ吹き出し、字幕、UIオーバーレイ、ロゴ、透かしは入れない。黒板、資料、端末、看板など場面内に自然な文字は必要最小限なら入れてよい。",
            ...
            "会話の直前ログ:",
            *recent_lines,
            "演出: 場所の特徴は背景で伝えつつ、キャラクターがそこで何を感じたかが一目で伝わる空気を作る。",
        ]
        prompt = "\n".join(lines)
        prompt = prompt_support.normalize_first_person_visual_prompt(prompt)
        prompt = prompt_support.apply_visual_style(prompt, context)
    ...
    def _build_lccd_photo_prompt(self, context: dict, instruction: str, pose_style: str, costume_image) -> str:
        ...
        prompt = "\n".join(
            [
                ...
                "衣装紹介だけで終わらせず、その衣装を見せることへの自信、照れ、期待、いたずらっぽさなど、キャラクターの感情が表情と視線に出る一枚にする。",
    ...
    def _build_photo_mode_prompt(
        ...
        prompt = "\n".join(
            [
                ...
                "キャラクターの感情が一目で分かる表情、視線、手元、肩や姿勢の変化を必ず描く。",
                "撮影ポーズだけで終わらせず、ユーザーに撮られていることへの反応、照れ、余裕、期待、緊張、安心などを顔と身体の演技で見せる。",
    ...
    def _build_lccd_room_prompt(self, context: dict, location_payload: dict) -> str:
        ...
        prompt = "\n".join(
            [
                ...
                "施設紹介画像ではなく、キャラクターが店に入って感情を動かした瞬間のイベントCGにする。",
                "主役は店内設備ではなく、キャラクターの反応。期待、照れ、自信、戸惑い、興味などを表情、視線、手元、肩や姿勢に出す。",
