(() => {
  ...
  const photobookGenerateButton = document.getElementById("feedPhotobookGenerateButton");
  const autoGenerateButtons = [autoGenerateSoloButton, autoGenerateDuoButton, photobookGenerateButton].filter(Boolean);
  ...
  async function generatePhotobookFeed(button = photobookGenerateButton) {
    const projectId = Number(composerProject?.value || projectFilter.value);
    const characterId = Number(composerCharacter?.value || characterFilter.value);
    if (!projectId) {
      NovelUI.toast("写真集モード用のワールドを選んでください。", "warning");
      return;
    }
    if (!characterId) {
      NovelUI.toast("写真集モード用のキャラクターを選んでください。", "warning");
      return;
    }
    const originalHtml = button?.innerHTML || "";
    autoGenerateButtons.forEach((item) => {
    ...
    if (button) {
      button.innerHTML = '<span class="spinner-border spinner-border-sm me-2"></span>写真集生成中...';
    }
    try {
      const body = {
        count: 1,
        interaction_mode: "photobook",
      ...
      NovelUI.toast("写真集Feedを生成しました。X予約は投稿カードの予約ボタンから設定できます。");
      if (projectFilter.value && Number(projectFilter.value) !== projectId) {
        projectFilter.value = String(projectId);
        await loadCharactersForProject(projectId, { target: characterFilter });
      }
  ...
  if (composerForm) {
    ...
    photobookGenerateButton?.addEventListener("click", () => {
      generatePhotobookFeed(photobookGenerateButton).catch((error) => NovelUI.toast(error.message || "写真集Feedの生成に失敗しました。", "danger"));
    });
