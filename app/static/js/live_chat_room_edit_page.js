(() => {
  const page = document.querySelector("[data-project-id][data-room-id]");
  ...
  const descriptionGenerateButton = document.getElementById("liveChatRoomDescriptionGenerateButton");
  ...
  function setDescriptionGenerating(active) {
    if (!descriptionGenerateButton) return;
    descriptionGenerateButton.disabled = active;
    descriptionGenerateButton.innerHTML = active
      ? '<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>生成中...'
      : '<i class="bi bi-stars"></i><span>自動入力</span>';
  }
  ...
  async function generateDescriptionDraft() {
    ...
    try {
      const draft = await NovelUI.api(`/api/v1/projects/${projectId}/chat/rooms/description-draft`, {
        method: "POST",
        body: {
      ...
      NovelUI.toast("ルーム紹介を自動入力しました。");
    } catch (error) {
      NovelUI.toast(error.message || "ルーム紹介の自動入力に失敗しました。", "danger");
    } finally {
  ...
  descriptionGenerateButton?.addEventListener("click", generateDescriptionDraft);
